    ...
    if (r + 1 < ROWS && board->avCells[r+1][c] && board->array[r+1][c] == -1) {
        for (int a = 0; a < 7; a++) {
            for (int b = a; b < 7; b++) {
                ...
                board->array[r][c] = a;
                board->array[r+1][c] = b;
                board->dominoMap[r][c] = board->dominoMap[r+1][c] = board->dominoId++;
                board->usedDomino[a][b] = true;
